...
public class StackSort {
    ...
    private static int[] doStackSort(int data[]) {
        ...
        for(int i = 0; i < data.length; i++) {
            ...
            k++;
            if(!lowervalues.isEmpty() && data.length > 1 && k < data.length-1 && data[k] == lowervalues.peek()) {
                lowervalues.push(data[i]);
            }

            if(!uppervalues.isEmpty() && data.length > 1 &&   k < data.length-1 && data[k] == uppervalues.peek()) {
                uppervalues.push(data[i]);
            }
