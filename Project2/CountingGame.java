...
public class CountingGame {
    ...
    public static void main(String args[]) {
        ...
        int rem = doRhyme(players, rhyme, position);

        while(!(players.getLength() == 1)) {
            doRhyme(players, rhyme, rem);
        }
    ...
    public static int doRhyme(ListInterface<Integer> players, ListInterface<String> rhyme, int startAt) {
        int lengthP = players.getLength();
        int lengthR = rhyme.getLength();
        int position = startAt;

        for(int i = 1; i < lengthR + 1; i++) {
            if(position > players.getLength())
                position = 1;
            System.out.println("Player " + players.getEntry(position) + ": " + rhyme.getEntry(i));
            position++;
        }
